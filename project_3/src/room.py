...
class Boundary:
    ...
    def __add__(self, other: Boundary):
        return Boundary(
...
class Room:
    ...
    def updateNeumannValues(self, temperature: np.ndarray, h):
        for i, (l, r) in enumerate(zip(self.left.dirichlet, self.right.dirichlet)):
            if l == 0:
                self.left.values[i] = h * (
                    -3 * temperature[i, 0]
                    + temperature[i + 1, 0]
                    + temperature[i - 1, 0]
                    + temperature[i, 1]
...
if __name__ == "__main__":
    ...
    for i in range(ITERATIONS):
        ...
        omega2.updateNeumannValues(t2, H)
        ...
        omega1.right.values[idx12] = omega2.left.values[idx21]
        t1, *_ = omega1.solveNeumann()
        omega2.left.values[idx21] = t1[idx12, -1]

        omega3.left.values[idx32] = omega2.right.values[idx23]
